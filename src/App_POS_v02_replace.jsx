...
export default function App() {
  ...
  async function initTerminal() {
    ...
    const { data: terminalRow, error } = await supabase.from('pos_terminals').select('*, branches(id,name,is_active)').eq('terminal_code', TERMINAL_ID).eq('is_active', true).maybeSingle()
    ...
    setBranch(terminalRow.branches || null)
    const { data: shiftRows } = await supabase.from('pos_shifts').select('*').eq('terminal_id', terminalRow.id).eq('status', 'open').order('opened_at', { ascending: false }).limit(1)
  ...
  if (screen === 'login') return <div className="login-screen"><div className="login-card"><ProductLogo /><p>Вход только по PIN. Сотрудник и позиция определяются автоматически.</p><div className="terminal-info"><span>Терминал: <b>{TERMINAL_ID}</b></span><span>Филиал: <b>{branch?.name || 'не привязан'}</b></span></div>{message && <div className="message bad">{message}</div>}<label>PIN</label><input value={'•'.repeat(pin.length)} readOnly placeholder="4 цифры" /><div className="pin-grid">{[1,2,3,4,5,6,7,8,9,'C',0,'⌫'].map(k => <button key={k} onClick={() => handlePin(k)}>{k}</button>)}</div><button className="primary full" onClick={loginByPin}>Войти</button></div></div>

  if (screen === 'shift') return <div className="login-screen"><div className="login-card shift-card"><ProductLogo /><h2>Кассовая смена</h2><p>Сотрудник: <b>{cashier?.full_name}</b> · {cashier?.position || 'позиция не указана'}</p><p>Филиал: <b>{branch?.name}</b></p>{message && <div className={`message ${message.includes('закрыта') ? 'good' : 'bad'}`}>{message}</div>}{!activeShift && <button className="primary full" disabled={loading} onClick={openShift}>{loading ? 'Открытие...' : 'Открыть кассовую смену'}</button>}{activeShift && <button className="primary full" onClick={() => setScreen('pos')}>Перейти в POS</button>}<button className="ghost full" onClick={logout}>Выйти</button></div></div>

  return (
    <div className="pos-app">
      <header className="top"><ProductLogo /><div className="top-meta"><b>{cashier?.full_name}</b><span>{cashier?.position || '—'} · {branch?.name || 'филиал не привязан'} · смена #{String(activeShift?.id || '').slice(0, 8)}</span></div><button onClick={loadMenu}>Обновить меню</button><button onClick={() => setScreen('shift')}>Смена</button><button onClick={logout}>Выход</button></header>
